...
function categoryVisualFor(category: string): CategoryVisual {
  ...
  const label = category.trim() || "Negócio local";

  if (/sal[aã]o|beleza|barbear|est[eé]tica|cabelo/.test(normalized)) {
    return { icon: Scissors, eyebrow: label, detail: "Imagem ilustrativa" };
  }
  if (/caf[eé]/.test(normalized)) {
    return { icon: Coffee, eyebrow: label, detail: "Imagem ilustrativa" };
  }
  if (/restaurante|comida|lanch|pizz|\bbar\b|padaria|confeitaria/.test(normalized)) {
    return { icon: UtensilsCrossed, eyebrow: label, detail: "Imagem ilustrativa" };
  }
  if (/hotel|pousada|hosped|hostel/.test(normalized)) {
    return { icon: BedDouble, eyebrow: label, detail: "Imagem ilustrativa" };
  }
  if (/cl[ií]nica|m[eé]dic|sa[uú]de|dent|odonto|fisi/.test(normalized)) {
    return { icon: HeartPulse, eyebrow: label, detail: "Imagem ilustrativa" };
  }
  if (/academia|fitness|crossfit|pilates|esporte/.test(normalized)) {
    return { icon: Dumbbell, eyebrow: label, detail: "Imagem ilustrativa" };
  }
  if (/pet|veterin|animal/.test(normalized)) {
    return { icon: PawPrint, eyebrow: label, detail: "Imagem ilustrativa" };
  }
  if (/loja|varejo|moda|boutique|mercado|com[eé]rcio/.test(normalized)) {
    return { icon: ShoppingBag, eyebrow: label, detail: "Imagem ilustrativa" };
  }
  return { icon: Building2, eyebrow: label, detail: "Imagem ilustrativa" };
}
...
export default async function DemoLandingPage({ params }: PageProps) {
  ...
  const buttonColor = readableAccentColor(content.primaryColor);
      ...
      <div className="relative z-10">
        ...
        <section id="inicio" className="relative scroll-mt-24">
          ...
          <aside className="relative mx-auto min-w-0 w-full max-w-xl lg:ml-auto">
            <div
              aria-hidden="true"
              className="absolute -inset-2 -z-10 rounded-[2.5rem] border border-white/[0.04]"
            />
            <div className="relative min-w-0 overflow-hidden rounded-[2rem] border border-white/10 bg-[#101016] sm:rounded-[2.5rem]">
              {heroImageUrl ? (
            ...
            <div className="relative z-10 mx-4 -mt-5 grid grid-cols-2 overflow-hidden rounded-2xl border border-white/10 bg-[#111118] sm:mx-7">
                ...
                {[
                  {
                    ...
                    detail: "Categoria informada na ficha",
                  },
                  {
                    icon: Palette,
                    eyebrow: "Negócio",
                    title: business.name,
                    detail: "Nome informado na ficha",
                  },
                  {
                    icon: MessageCircle,
                    eyebrow: "Localização",
                    title: locationSummary || "Não informada",
                    detail: "Dado disponível na ficha",
                  },
                ].map(({ icon: VisualIcon, eyebrow, title, detail }, index) => (
                  <figure
                    key={`${index}-${title}`}
                    className="group relative min-h-[25rem] overflow-hidden rounded-[1.75rem] border border-white/[0.09] bg-[#101016] p-6"
                    role="img"
                    aria-label={`Imagem ilustrativa: ${eyebrow}`}
                    style={{ borderTopColor: index === 1 ? content.accentColor : content.primaryColor }}
                  >
                ...
                {content.processSteps.map((step, index) => (
                  ...
                  >
                    <span
                      ...
                      style={{ backgroundColor: `${content.primaryColor}2c`, color: accentTextColor }}
                    >
                      {String(index + 1).padStart(2, "0")}
                    </span>
              ...
              >
                {osmLinks ? (
                  <>
                    ...
                    />
                    <div className="pointer-events-none absolute inset-x-0 top-0 h-28 bg-black/30" />
        ...
        <section className="mx-auto max-w-[70rem] px-5 py-20 sm:px-8 sm:py-24 lg:px-10">
          <div
            ...
            style={{ backgroundColor: `${content.primaryColor}18` }}
          >

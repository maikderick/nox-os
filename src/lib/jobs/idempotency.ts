...
/**
 * How long a reservation holds. Fixed, and not a parameter.
 *
 * It was configurable, and that was a footgun with no upside: a caller
 * passing a TTL shorter than its own execution would have its reservation
 * expire mid-work, a second caller would take over, and the mechanism built
 * to prevent duplicate work would be the thing scheduling it. Nobody needs a
 * shorter one, so nobody may ask for one.
 */
export const DEFAULT_TTL_SECONDS = IDEMPOTENCY_TTL_SECONDS;
...
export async function withIdempotency<T>(
  ...
  // And so is the classification, for a sharper reason than symmetry.
  //
  // Letting the stored value simply win would mean a caller arriving with an
  // `ExternalWork` — a zero-argument function that talks to a provider — being
  // dispatched down the `LOCAL` path because the row says so. It would then be
  // called with a transaction client it ignores, and if it threw, the key would
  // be released: the safe thing to do after a local rollback, and the worst
  // possible thing to do after a remote effect.
  //
  // Refused outright, and nothing is touched — not executed, not reconciled,
  // not concluded, not released, not even re-stamped. Only a stored value we
  // do not recognise falls through, and that keeps going to the strictest path.
  if (isSideEffect(existing.sideEffect) && existing.sideEffect !== params.sideEffect) {
    throw new JobRefusal("CLASSIFICACAO_DIVERGENTE");
  }
 ...
 *
 * By the time this runs, the stored declaration and the caller's have already
 * been checked against each other, so they either agree or the stored one is a
 * value this version does not recognise. The unrecognised case reads as the
 * strictest one — a row written by something we do not understand is not a row

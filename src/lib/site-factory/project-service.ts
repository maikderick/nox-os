...
export async function listSiteProjects(actor: Actor) {
  ...
  return prisma.siteProject.findMany({
    ...
    include: {
      ...
      repository: { select: { protectedAt: true } },
  ...
  const project = await prisma.siteProject.findFirst({
    ...
    include: {
      ...
      generationRuns: {
        ...
        include: {
          reservation: { select: { status: true, amountCents: true } },
...
export async function transitionSiteProject(params: {
  ...
  // Both checked after authorization so an unauthorized caller still gets 403,
  // and before any write so a refused stage leaves the project exactly as it
  // was.
  if (isStagePendingOrchestrator(params.to)) {
    throw new SiteProjectStageUnavailableError(params.to);
  }
  if (isStageRequestedNotTransitioned(params.to)) {
    throw new SiteProjectStageNotTransitionableError(params.to);
  }

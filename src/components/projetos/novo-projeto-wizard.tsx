      ...
      <ol className="mb-6 grid grid-cols-5 gap-2" aria-label="Etapas do projeto">
        {STEPS.map((label, index) => (
          <li key={label} className="min-w-0" aria-current={index === step ? "step" : undefined}>
            <div
              className={`h-1 rounded-full ${index <= step ? "bg-nox-cyan" : "bg-nox-border"}`}
            />
      ...
      <div className="mt-5 flex items-center justify-between gap-4">
        ...
        {step < STEPS.length - 1 ? (
          ...
          <button
            ...
            onClick={() => void submit()}
            aria-disabled={stepIssues.length > 0}
            className={`inline-flex items-center gap-2 rounded-xl bg-nox-cyan px-5 py-2.5 text-sm font-semibold text-nox-bg disabled:opacity-40 ${stepIssues.length > 0 ? "opacity-40" : ""}`}
          >
            {submitting ? (

...
describeLocalDatabase("one key, one execution", () => {
  ...
  describe("an expired reservation, and what its declared effect allows", () => {
    ...
    it("refuses a caller whose declaration disagrees with the stored one", async () => {
      // Letting the stored value simply win is not enough, and the direction
      // that shows why is stored `LOCAL` with an external caller: the external
      // work would run down the local path and its throw would release the
      // key — after a possible remote effect. Both directions are refused
      // outright, in `jobs-idempotency-limits.test.ts`.
      const key = randomUUID();
      await seed({ key, sideEffect: "EXTERNO_AMBIGUO" });

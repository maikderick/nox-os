...
describe("two doors, and neither opens the other", () => {
  ...
  it("is not scheduled by Vercel, and must not go back to being", async () => {
    // The account is on Hobby, where cron is daily at best — and a `crons`
    // block there makes Vercel reject the deployment before the build and the
    // migrations even run. The every-minute schedule lives in
    // `infra/aws-cron/` instead. This pins the rule so the block does not
    // drift back in.
  ...
  it("is woken by a scheduler that speaks the door it actually has", async () => {
    // The external scheduler has to use `GET` with the cron credential. A
    // `POST` would land on the person-door, which has no cron path at all and
    // answers 401 — the queue would simply never run, once a minute, forever.
    const { readFileSync } = await import("node:fs");
    const handler = readFileSync("infra/aws-cron/handler.py", "utf8");

    expect(handler).toContain('method="GET"');

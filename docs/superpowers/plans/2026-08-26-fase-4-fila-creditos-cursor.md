...
requestHash, sideEffect }, work)`.

O TTL não é parâmetro: é fixo, e derivado do teto de execução da função em
`limits.ts`. Uma reserva mais curta que a execução que ela protege é pior que
nenhuma — vence com o trabalho ainda rodando, alguém assume, e o mecanismo
feito para evitar trabalho duplicado passa a agendá-lo.
